...
fn main() {
    ...
    let mut __evo_item = __EvoRecord_Item {
        __evo_field_value: __evo_seed,
    };
    let mut __evo_sum = 0;
    for _ in 0..__evo_n {
        __evo_sum = (__evo_sum + __evo_fn_read_value(&__evo_item));
        if ((__evo_item).__evo_field_value > 1000) {
            __evo_item = __EvoRecord_Item {
                __evo_field_value: ((__evo_item).__evo_field_value / 2),
            };
        } else {
            __evo_item = __EvoRecord_Item {
                __evo_field_value: ((__evo_item).__evo_field_value + 7),
            };
        }
    }
    println!("{}", __evo_sum);
