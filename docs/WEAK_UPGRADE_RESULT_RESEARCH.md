...
Acceptance rule: the research decision is authoritative only when the executable matrix, exact-head CI, artifact provenance, merge, and natural exact-main validation recorded on issue #138 are all green. Until then, the production consumer #137 remains blocked.
...
- `weak`, `downgrade`, `upgrade`, and `as` remain contextual identifiers outside their bounded grammar positions; `else` and `end` reuse the existing block delimiters;
...
The dedicated workflow derives the decision from measured matrix/surface predicates and must emit exactly one #138 outcome. It may emit `SCOPED-UPGRADE-CANDIDATE` only if:
