...
struct Case {
    ...
    family: Family,
    compile: bool,
    compile_stderr_contains: Option<&'static str>,
...
const CASES: &[Case] = &[
    ...
    Case {
        ...
        family: Family::ScopedUpgrade,
        compile: false,
        compile_stderr_contains: Some("error[E0382]"),
        stdout: None,
...
fn surface_evidence() -> SurfaceEvidence {
    let contextual =
        lex("weak downgrade upgrade edge as owner\n").expect("contextual source should lex");
    ...
    let contextual_words_are_identifiers =
        identifiers.as_slice() == ["weak", "downgrade", "upgrade", "edge", "as", "owner"];
    ...
    let scoped_upgrade_current_parser_rejects = lex(scoped).is_ok_and(|tokens| {
    ...
    let maybe_shared_type_current_parser_rejects = lex(maybe_shared).is_ok_and(|tokens| {
    ...
    let generic_option_current_parser_rejects = lex(generic_option).is_ok_and(|tokens| {
        parse(&tokens).is_err_and(|error| {
            error
                .message
                .contains("expected ')' after function parameters")
    ...
    let enum_shared_owner_currently_fail_closed =
        lex(enum_shared).is_ok_and(|tokens| match parse(&tokens) {
            Err(error) => error
                .message
                .contains("shared-owner enum payloads are not supported in v0"),
            Ok(program) => lower(&program).is_err_and(|error| {
                error
                    .message
                    .contains("enum payloads are not supported in v0")
            }),
        });
...
fn weak_upgrade_result_surface_research_selects_scoped_upgrade_candidate() {
    ...
    let scoped_semantics_satisfied = CASES
        .iter()
        .zip(&findings)
        .filter(|(case, _)| case.family == Family::ScopedUpgrade)
        .all(|(_, finding)| finding.compile_matched && finding.runtime_matched);
    let first_class_result_required = !scoped_semantics_satisfied;
    let verdict = if scoped_semantics_satisfied
        && weak_upgrade_non_consuming
    ...
    {
        "SCOPED-UPGRADE-CANDIDATE"
    } else {
        "DEFER"
    };
    let recommended_surface = if verdict == "SCOPED-UPGRADE-CANDIDATE" {
        "SCOPED-UPGRADE-BRANCH"
    } else {
        "NONE"
    };
    assert_eq!(verdict, "SCOPED-UPGRADE-CANDIDATE");
    ...
    writeln!(md, "- verdict: **{verdict}**").unwrap();
    writeln!(md, "- recommended surface: **{recommended_surface}**").unwrap();
