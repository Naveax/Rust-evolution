...
spec = Path("docs/LANGUAGE_SPEC_V0.md")
text = spec.read_text()
replacements = [
    (
...
for old, new in replacements:
    if text.count(old) != 1:
        raise SystemExit(f"LANGUAGE_SPEC_V0 anchor count for {old[:40]!r}: {text.count(old)}")
    text = text.replace(old, new, 1)
spec.write_text(text)

Path("docs/NEXT_ACTION.md").write_text("""# Rust Evolution — NEXT ACTION
...
start = text.index("## Active research — #102 / PR #103 immutable reference surface v0")
end = text.index("## CI / handoff invariant")
replacement = """## #102 / PR #103 — immutable reference surface research completed
